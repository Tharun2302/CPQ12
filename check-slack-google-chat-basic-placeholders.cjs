...
async function checkPlaceholders() {
  const templatePath = path.join(__dirname, 'backend-templates', 'slack-to-google-chat-basic.docx');
  
  console.log('🔍 Checking placeholders in Slack to Google Chat Basic template...\n');
  ...
  console.log(`   Match rate: ${((expectedPlaceholders.length - missing.length) / expectedPlaceholders.length * 100).toFixed(1)}%`);
