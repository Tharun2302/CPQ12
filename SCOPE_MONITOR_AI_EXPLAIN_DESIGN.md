...
**Closed 2026-09-11**, across two passes. The second pass existed because the first introduced three regressions of its own — recorded here because the pattern matters more than the individual bugs: every one was verified at the wrong layer, by a test that exercised a function directly while production reached it through a different path.

1. `renderAiMessage()` redacts every raw line and the `Container:` line before the 280-char cut. It was the one Teams path with no masking, verified leaking passwords, customer emails and JWTs.
2. A group with no explanation is disclosed and its raw line still bulleted, instead of vanishing while the header counted it. `guardDropped` in the payload reports the **real** guard figure threaded from `guardedPrompt()` — not the size of the unexplained list, which also catches budget-trimmed groups and short provider replies. The Teams notice is deliberately cause-neutral ("not explained this scan"): naming the guard would both be wrong for those cases and advertise which lines are secret-bearing in a searchable channel.
3. `MONGO_HOST` no longer requires a 6-character middle label. Real Atlas project ids are 5, so live cluster hostnames went unmasked — which then tripped the guard and, via item 2, disappeared.
4. **`splitLines()` no longer caps.** `parseLogLine` receives the line intact, so `source: 'client'` survives and `isAiEligible` refuses the entry exactly as designed, while `entry.raw`/`entry.message` are still capped at 8192 — the bound now sits where it is needed rather than where it destroyed information. `bullet()` and `reportLines()` cap their own input for the same reason — the latter took an 8.6 MB fixture's on-disk report from 8,600,518 bytes to 8,565.

   **Three attempts, recorded because the failure mode repeated.** (a) Parse-before-cap alone did nothing in production: `splitLines()` capped first, and the unit test passed only because it called `parseLogLine` directly. (b) Refusing truncated lines AI *eligibility* closed the bypass but recreated item 2's disappearance — a genuine 14 KB `MongoServerSelectionError` became no group, so it was not in `withheld` either and never appeared in the alert while the header counted it. (c) Withholding them in `guardGroups` restored visibility but let eight padded client entries form all eight groups and starve real faults, reintroducing the slot-exhaustion attack from an unauthenticated endpoint — and counted each truncation withhold as a redaction-guard drop, reopening item 2's miscount. Only moving the bound closes all of it, because the distinguisher the check needs — the `source` tag — is precisely what the cap was destroying.

   Verified flat from 8 KB to 3 MB, and an 8.6 MB single line runs in 138-215 ms at ~56 MB peak RSS with `entry.raw` bounded at 8,218.

   **State the property precisely**, because the obvious shorthand is false: it is *not* true that nothing unbounded reaches a regex. An uncapped line is still matched in full by every `NOISE_PATTERN`, every `USER_ACTIVITY_PATTERN`, the first matching `GENERIC_PATTERN`, the `/
?
/` split and `DOCKER_CLI_STREAM_ERRORS` — 14 regexes, 27 on the docker path. The correct claim is: **every super-linear regex is bounded to 8,232 chars; the full-line matchers that remain are literal or `^`-anchored and linear.** Measured on adversarial input: 8 MB of leading whitespace against `/^\s*error from daemon in stream:/i` is 79 ms, 8 MB of `❌ Errors: 01` near-misses is 135 ms.
5. `safeOriginLabel()` (now in `cors-origin-utils.cjs`, so tests need not boot `server.cjs`) allows only `[A-Za-z0-9.:/_-]{1,120}` and substitutes `(unprintable origin)`. Which origins are *allowed* is unchanged.
6. `capFrames()` recognises the standard `at fn (/path:1:2)` frame by requiring a **contiguous** path-then-`:line:col` call site. Testing for a separator and a `:d:d` separately matched prose like "failed at 10:00:00 for /api/quotes".
7. `intFromEnv` validates with `Number` + `Number.isInteger` + explicit range on all ten `AI_*` limits. `AI_MAX_CHARS_PER_ERROR` has an 80 floor: at 0 the per-group guard inspected empty strings and passed everything while `buildPrompt`'s `|| 400` fallback still sent 400 real characters — a clean guard bypass. Both `AI_MAX_MESSAGE_CHARS` readers now share the 500-28000 range.
8. `DOCKER_CLI_STREAM_ERRORS` tolerates leading whitespace. The two generic wordings added in the first pass (`context canceled`, `error: unexpected EOF`) were **removed**: a match forces DEGRADED and skips the AI layer, and `server.cjs` has 284 plain `console.error` calls that reach stderr unwrapped — so "context canceled while streaming the Gotenberg PDF response" would report a real fault as a broken monitor. The `^` anchor stays: app lines are `JSON.stringify`'d, so attacker text inside a `{…}` line cannot impersonate a CLI error.
9. `capScanLine()` is idempotent, gated on **length as well as the marker**. Testing the marker alone was attacker-forgeable — a 3 MB line ending in it came back uncapped, defeating the bound for `redact()`, `fingerprint()`, `classify()` and `bullet()` on every scan, AI or not.
10. `timestamp`, `level` and `source` are capped at 120 chars. Also in this round: all CORS rejections now fingerprint to a single group, because the origin is unauthenticated attacker input and distinct origins would otherwise fill all eight slots and burn the daily call budget; and `fail()` carries the real `guardDropped`, so the total-drop case no longer reports 0 while `aiStatus` says `failed_redaction_guard`. A 200 K-char fractional-second timestamp satisfied `ISO_TIMESTAMP`, counted nothing against `maxTotalChars`, and inflated a prompt to 400,284 characters against a 6,000 cap.

**Rejected, with reasons:**

- **Keeping `:line:col` in `[SRC:…]`.** Suggested as more actionable. It contradicts the requirement that no path, line number or dependency layout leaves the box, and three tests encode that intent. Not worth overriding a security decision for a marginal readability gain.

**Still open:**

- **`spawn` + `readline` with a bounded ring buffer.** Better than a `maxBuffer` ceiling: on ENOBUFS the whole window is discarded, and a log storm is exactly when the window matters. A redesign, not a fix.
- **The withheld population is, by construction, what `redact()` cannot mask.** `RESIDUAL_SHAPES` is deliberately not a subset of `REDACTIONS`, so a group is withheld precisely because it carries a shape with no masking rule — GitHub PATs, card numbers, SSNs among them. `rawBullet` redacts, but cannot mask what has no rule. **Correction to an earlier claim here:** this is *mostly* parity with `buildMessage()`, which has always leaked the same content — but only for `level: error` lines, whose JSON matches `/ERROR/i`. A `level: warn` line carrying an unmasked secret is posted by the withheld renderer and was not posted by the fallback, so the AI path does add a small new class. Still accepted: same internal channel, and the alternative was errors vanishing silently. The real fix is to extend `REDACTIONS` over the unmasked `RESIDUAL_SHAPES` (`ghp_`/`gho_`, `glpat-`, `npm_`, `AIza`, `sk_live`, PEM headers, SSN, card numbers), which would make both Teams paths sufficient.
- **Golden-fixture coverage.** `skeleton()` blanks bullet content, so 57 of 90 cases compare layout only and over-masking passes silently.
- **`fingerprint` rows of the growth check are inert** — it caps internally, so both input sizes do identical work and the ratio is always ~1.
- **Timing-test flake risk.** One unreproduced failure was seen. The absolute ceilings have 17x headroom; the growth ratios have 2.6x and are the tighter constraint, and `Math.max(bestMillis(...), 0.5)` turns a floored baseline into a 12.5 ms absolute ceiling — squarely in scheduler-noise range. Sizing the small input above timer noise, or skipping the ratio when the baseline floors, would fix it. Vitest cannot interrupt a synchronous regex, so a real quadratic regression wedges the worker rather than failing.
- **`AI_MAX_GROUPS` accepts up to 100** while `RESPONSE_SCHEMA.maxItems` is 8; above 8 the surplus lands in `withheld` and is raw-bulleted. Capping at 8 would be tidier. The withheld section rendering last — and so being first cut by the message cap — is the same defect: unreachable at the default 8 (at most 7 groups can be withheld, giving ~2.3 KB against an 8000 cap), and only reachable through this setting.
- **`REPORT_DIR` has no retention of any kind.** Nothing in the repo ever deletes from `logs/monitor/`; the monitor writes one file per run and leaves it. Compare `server/logger.cjs`, which has both a 50 MB/day cap and a 2-day sweep — the monitor's own output has neither. Pre-existing and unrelated to these passes, but it is the larger of the two disk risks and deserves its own ticket. (Per-line size *is* now bounded: `reportLines()` caps each line, which took an 8.6 MB fixture's report from 8,600,518 bytes to 8,565.)
- **`selectMessage`'s render-failure downgrade drops `guardDropped`**, consistent with severity and summary also going null. Fine unless the number is ever treated as an audit figure.
- **`capFrames` undercounts pathless V8 frames** (`at async Promise.all (index 0)`, `at Array.forEach (<anonymous>)`). Cosmetic, unchanged from before, not worth fixing.
- **The `capFrames` growth test uses the cheapest input shape** (`' at boom '`, no slashes). The expensive shape is slash-dense at roughly 10x the cost, so the test would not notice a `CALL_SITE` rewrite that reintroduced backtracking.
