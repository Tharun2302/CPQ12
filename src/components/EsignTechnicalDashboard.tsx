...
const EsignTechnicalDashboard: React.FC = () => {
  ...
  const tabs = [
    { id: 'queue', label: 'My E-Sign Queue', icon: User, count: queueCount },
    { id: 'status', label: 'E-Sign Status', icon: BarChart3, count: queueCount },
  ];

  return (
    <div className="min-h-screen bg-gray-50">
      <Navigation currentTab="esign-technical" />
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            <div className="text-center">
              <div className="flex items-center justify-center gap-3 mb-2">
                ...
                <h1 className="text-4xl font-bold text-gray-900">E-Sign Technical Dashboard</h1>
              </div>
              <p className="text-xl text-gray-600">Technical e-signature: sign or review documents (fixed roles: Team → Tech → Legal → Deal Desk)</p>
              ...
              <div className="flex items-center justify-between gap-3 flex-wrap">
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 bg-blue-100 rounded-lg flex items-center justify-center">
                    {React.createElement(tabs.find(t => t.id === activeTab)?.icon || User, { className: 'w-4 h-4 text-blue-600' })}
                  </div>
            ...
            <div className="p-6">
              ...
              {!loading && !error && userEmail && items.length === 0 && (
                <div className="text-center py-12">
                  ...
                  <p className="text-gray-400 text-sm mt-1">When a document reaches the Technical step (fixed roles), it will appear here.</p>
                </div>
              )}
                    ...
                    <div key={item.signing_token} className="bg-white border border-gray-200 rounded-lg p-6 shadow-sm">
                      ...
                      <div className="mb-4">
                        <div className="flex items-center justify-between text-sm text-gray-600 mb-2">
                          <span>Status</span>
                          <span className="text-blue-600 font-semibold">Pending your signature</span>
                        </div>
                        <div className="w-full bg-gray-200 rounded-full h-2">
                          <div className="bg-blue-500 h-2 rounded-full" style={{ width: '50%' }} />
                        </div>
                      </div>
                      <div className="flex items-center gap-3 text-sm p-2 rounded bg-blue-50 border border-blue-200">
                        <span className="font-medium">Technical Team</span>
                        <span className="text-gray-500">{userEmail}</span>
                        <span className="text-blue-600 font-semibold text-xs">(Your Turn)</span>
                      </div>
...
export default EsignTechnicalDashboard;
