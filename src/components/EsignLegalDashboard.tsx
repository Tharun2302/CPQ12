...
import { FileText, Loader2, PenLine, User, RefreshCw, Crown, BarChart3, Eye, X } from 'lucide-react';
...
const EsignLegalDashboard: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <Navigation currentTab="esign-legal" />

      <div className="lg:pl-64">
        {/* Header - same pattern as Legal Team Approval Portal */}
                ...
                return (
                  <button
                    ...
                    className={`flex items-center gap-2 px-4 py-3 rounded-lg font-medium text-sm whitespace-nowrap transition-all duration-200 ${
                      isActive ? 'bg-purple-600 text-white shadow-md' : 'text-gray-600 hover:text-gray-900 hover:bg-gray-50'
                    }`}
                  >
        ...
        {/* Content - card layout like Legal Team Approval Portal */}
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
              ...
              <div className="flex items-center justify-between gap-3 flex-wrap">
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 bg-purple-100 rounded-lg flex items-center justify-center">
                    {React.createElement(tabs.find(t => t.id === activeTab)?.icon || Crown, { className: 'w-4 h-4 text-purple-600' })}
                  </div>
                    ...
                    <div key={item.signing_token} className="bg-white border border-gray-200 rounded-lg p-6 shadow-sm">
                      ...
                      <div className="mb-4">
                        <div className="flex items-center justify-between text-sm text-gray-600 mb-2">
                          <span>Status</span>
                          <span className="text-purple-600 font-semibold">Pending your signature</span>
                        </div>
                        <div className="w-full bg-gray-200 rounded-full h-2">
                          <div className="bg-purple-500 h-2 rounded-full" style={{ width: '75%' }} />
                        </div>
                      </div>
                      <div className="flex items-center gap-3 text-sm p-2 rounded bg-purple-50 border border-purple-200">
                        <span className="font-medium">Legal Team</span>
                        <span className="text-gray-500">{userEmail}</span>
                        <span className="text-purple-600 font-semibold text-xs">(Your Turn)</span>
                      </div>
      ...
      {/* Document Preview Modal - same pattern as Legal Team Approval (Approve/Deny → Sign/Review) */}
...
export default EsignLegalDashboard;
