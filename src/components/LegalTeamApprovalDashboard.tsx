    ...
    try {
      ...
      if (workflow) {
        ...
        const response = await fetch(`${BACKEND_URL}/api/send-deal-desk-email`, {
          ...
          body: JSON.stringify({
            dealDeskEmail: workflow.workflowSteps?.find(step => step.role === 'Deal Desk')?.email || 'salesops@cloudfuze.com',
            workflowData: {
        ...
        if (result.success) {
          ...
          try {
            await updateWorkflowStep(workflowId, 4, { status: 'approved', comments: 'Notified' });
          } catch (e) {
